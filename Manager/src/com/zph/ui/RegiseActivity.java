	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == login) {
			...
			if ("".equals(JPassword1.getText())) {
				JOptionPane.showConfirmDialog(null, "重复密码不能为空", "确认",
						JOptionPane.YES_NO_OPTION);
				return;
			}
			if (!JPassword.getText().equals(JPassword1.getText())) {
				JOptionPane.showConfirmDialog(null, "两次密码不一致", "确认",
						JOptionPane.YES_NO_OPTION);
				return;
			}
			String message = loginService.Reigise(JName.getText(),
					JPassword.getText());
			...
			if ("OK".equals(message)) {
				...
				JOptionPane.showConfirmDialog(null, "注册失败！", "确认",
						JOptionPane.YES_NO_OPTION);
			}
